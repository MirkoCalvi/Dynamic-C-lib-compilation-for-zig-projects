...
pub fn build(b: *std.Build) void {
    ...
    exe.addIncludePath(b.path("./src/menus/c_lib"));
    //exe.linkLibC();
    exe.addCSourceFile(.{ .file = b.path("./src/menus/c_lib/normal_lib.c"), .flags = &.{"-std=c99"} });
    // exe.addCSourceFile(.{ .file = b.path("src/menus/c_lib/vegan_lib.c"), .flags = &.{"-std=c99"} });
    // exe.addCSourceFile(.{ .file = b.path("src/menus/c_lib/megan_lib.c"), .flags = &.{"-std=c99"} });
    // exe.addCSourceFile(.{ .file = b.path("src/menus/c_lib/fruitarian_lib.c"), .flags = &.{"-std=c99"} });
